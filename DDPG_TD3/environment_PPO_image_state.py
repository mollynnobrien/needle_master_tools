...
class Environment:
	...
	def __init__(self, action_dim, log_f, img_stack, filename=None, mode=mode_demo, device=torch.device('cpu')):
	...
	def reset(self, log_f):
		...
		ob, state, _, _ = self.step(action_ini, log_f)
	...
	def step(self, action, log_f):
		...
		ob = torch.FloatTensor(self.stack).permute(1, 0, 2, 3)
		return ob, state, reward, not running
