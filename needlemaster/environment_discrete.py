...
class Environment:
    ...
    def reset(self, random_needle=False):
        ...
        if self.mode == 'rgb_array':
            return ob
        elif self.mode == 'gray_array':
	        frame = self.render(save_image=False)
	        gray = rgb2gray(frame)
	        self.stack = [gray] * self.stack_size  # four frames for decision
    ...
    def render(self, mode='rgb_array', save_image=False, save_path='./out/'):
        ...
        if mode in ['rgb_array', 'gray_array']:
    ...
    def step(self, action_idx):
        ...
        if self.mode == 'rgb_array':
            ...
            return torch.FloatTensor(state), reward, done
        elif self.mode == 'both':
