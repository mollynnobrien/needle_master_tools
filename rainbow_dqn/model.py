...
class DQN_state(nn.Module):
  def __init__(self, state_dim, args, action_space):
    super().__init__()
    self.atoms = args.atoms
    self.action_space = action_space
    self.state_dim = state_dim

    self.fc_h_v = NoisyLinear(self.state_dim, args.hidden_size, std_init=args.noisy_std)
    self.fc_h_a = NoisyLinear(self.state_dim, args.hidden_size, std_init=args.noisy_std)
    self.fc_z_v = NoisyLinear(args.hidden_size, self.atoms, std_init=args.noisy_std)
    self.fc_z_a = NoisyLinear(args.hidden_size, action_space * self.atoms, std_init=args.noisy_std)
