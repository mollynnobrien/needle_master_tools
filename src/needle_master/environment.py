...
class Environment:
    ...
    def compute_passed_gates(self):
        ...
        for gate in self.gates:
            thread_the_needle = [gate.box.contains(pt) for pt in self.thread_points]
            if(np.sum(thread_the_needle) > 0):
    ...
    def damage_score(self):
        ...
        damage = 0
        for surface in self.surfaces:
            surface_damage, surface_hit = surface.get_damage(self.thread_points)
            if(surface.deep_tissue and hit):
                surface_damage = surface_damage - 1000
            damage = damage + surface_damage
